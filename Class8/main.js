...
function setupCanvas(barchartData,moviesClean) {
    ...
    function update(data){
        xMax = d3.max(data, d => d[metric]); // 找到資料當中的最大值
        //V3 簡寫
        xScale_v3 = d3.scaleLinear([0, xMax], [0, chart_width]);
        
        yScale = d3.scaleBand().domain(data.map(d => d.title)).rangeRound([0, chart_height]).paddingInner(0.25);
        ...
        bars.selectAll('.bar').data(data,d=>d.title).join(
            enter =>{
                enter.append("rect").attr('class','bar').attr('x',0).attr('y', d => yScale(d.title))
                .attr('height',yScale.bandwidth()).style('fill','lightcyan').transition(transitionDelay)
                .delay((d,i)=>i*20).attr('width',d=>xScale_v3(d[metric])).style('fill','dodgerblue');

            },
            update =>{
                update.transition(defaultDelay).delay((d,i)=>i*20)
                .attr('y',d=>yScale(d.title)).attr('width',d=>xScale_v3(d[metric]))
            },
            exit => {
    ...
    const svg_width = 700; //畫布寬度
    const svg_height = 500; //畫布高度
    const chart_margin = { top: 80, right: 80, bottom: 40, left: 250 }; //畫布離邊緣的距離
    ...
    const xExtent = d3.extent(barchartData, d => d[metric]); // return array 找到資料當中的最大、最小值
    ...
    //V2 (x座標從0開始)
    let xMax = d3.max(barchartData, d => d[metric]); // 找到資料當中的最大值
